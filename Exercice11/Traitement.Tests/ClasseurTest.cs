...
namespace Traitement.Tests
{
    ...
    public class ClasseurTest : IDisposable
    {
        ...
        public void Creer_Nominal_Succes()
        {
            ...
            // Execution
            var actual = cible.Creer(nomDossier, fichiers);

            // Vérification
            Verify(attendu, actual);
        }
        ...
        public void Creer_MauvaisFormatNomDossier_Erreur()
        {
            ...
            // Execution
            Action actual = () => cible.Creer(nomDossier, fichiers);

            // Vérification
            actual.Should().Throw<BusinessException>(attendu).And.Message.Should().Be(Resources.MessageMauvaisFormatDossier);
        }
